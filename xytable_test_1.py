...
def table_state_machine(xy_table, welder, adam, current_state: int, table_of_positions: list, table_index: int, welding_position: int) -> Tuple[int, int, int]:
    ...
    match current_state:
        case -1:  # Handle case light curtain interrupted when xytable is homing.
                ...
                if current_pos != (0, 0):
                    ...
                    if is_move_button_pressed(adam):
                        ...
                        xy_table._recovery_context = {
                            ...
                            "return_state": 0,       # go to state 0 after recovery
                            "advance_table": False,
                        }
        ...
        case 30:
            ...
            if ctx is None:
                ...
                button = wait_for_move_or_grind(adam)  # blocking — waits for operator

                if button == 'grind':
                    # Operator wants to grind the electrode before continuing.
                    # State 31 handles: home → grind position → wait → return to target.
                    # _recovery_context is intentionally left unchanged so state 31 can read it.
                    print("Routing to grind recovery state.")
                    _next_state = 31
        ...
        case 31:
            ...
            # ---------------------------------------------------------------
            # Entered from state 30 when the operator presses Grind instead
            # of Move after a power cut (sticky electrode scenario).
            #
            # Sequence:
            #   1. Home — mandatory after power cut to re-reference zero.
            ...
            print("🔧 Grind recovery: homing first to re-reference zero...")
            ctx = xy_table._recovery_context
            if ctx is None:
                print("⚠️  Grind recovery entered with no context — resetting to state 0.")
                _next_state = 0
            else:
                target_x   = ctx["target"][0]
                target_y   = ctx["target"][1]
                return_state = ctx["return_state"]
                ...
                if xy_table._power_was_cut:
                    print("⚠️  Power cut during home (grind recovery). Back to state 30.")
                    _next_state = 30
                elif not home_ok:
                    print("⚠️  Homing failed (grind recovery). Back to state 30.")
                    _next_state = 30
                else:
                    # Step 2: Move to grinding position
                    print(f"✅ Homed. Moving to grind position "
                          f"X={GRIND_POSITION_X} Y={GRIND_POSITION_Y}...")
                    xy_table._power_was_cut = False
                    ...
                    if xy_table._power_was_cut:
                        ...
                        # Step 3: Wait for operator to finish grinding
                        print("🔧 Grind position reached. Operator: grind the electrode,")
                        print("   then press GRIND button to continue.")
                        is_grind_button_pressed(adam)

                        # Step 4: Move back to the original target
                        print(f"✅ Grinding done. Moving to X={target_x} Y={target_y}...")
                        xy_table._power_was_cut = False
                        xy_table.goto_position(target_x, target_y, units_in_mm=True)

                        if xy_table._power_was_cut:
                            print("⚠️  Power cut moving to target after grinding. Back to state 30.")
                            _next_state = 30
                        else:
                            current_pos = xy_table.positions_in_mm
                            if is_position_close_to(current_pos, (target_x, target_y), 0.02):
                                # Step 5: Return to state machine — do NOT increment table_index.
                                # State 3 will re-evaluate whatever table_index points to:
                                # already-at-position → natural advance to WELD command, or
                                # directly a WELD command → fires immediately.
                                print(f"✅ Grind recovery complete — at X={target_x} Y={target_y}.")
                                xy_table._recovery_context = None
                                _next_state = return_state
                            else:
                                print(f"⚠️  Off-target after grind recovery "
                                      f"(got {current_pos}). Retrying from state 31.")
                                _next_state = 31
