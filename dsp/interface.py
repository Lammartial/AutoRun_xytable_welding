...
def get_parameter_for_testrun(test_type: str, station_id: str, line_id: str, test_socket: str) -> dict:
    global API_URL

    response = requests.get(f"{API_URL}/parameter/{test_type}/{station_id}/{line_id}/{test_socket}")
    # expects JSON of
    # {
...
def ifc_get_parameter_for_testrun(test_type: str, station_id: str, line_id: str, test_socket: str) -> tuple:
    ...
    d = get_parameter_for_testrun(test_type, station_id, line_id, test_socket)
...
def send_result_of_testrun(result_list: list[dict]) -> list[dict]:
    global API_URL

    _remaining_list = []
    for result in result_list:
        response = requests.post(f"{API_URL}/result", json=result)
        if response.status_code != 200:
...
def load_result_list_from_json(filepath: str | Path) -> list[dict]:
    _local_result_file = Path(filepath)
...
def save_result_list_to_json(result_list: list[dict], filepath: str | Path) -> None:
    _local_result_file = Path(filepath)
    with open(_local_result_file, "wt") as out_file:
...
def ifc_send_result_for_testrun(result: tuple) -> None:
    global LOCAL_RESULT_FILE
    ...
    result_list = load_result_list_from_json(LOCAL_RESULT_FILE)
    result_list.append(result)
    remaining_list = send_result_of_testrun(result_list)
    save_result_list_to_json(remaining_list, LOCAL_RESULT_FILE)
...
if __name__ == "__main__":
    ...
    test_run = get_parameter_for_testrun("CORE_PACK_TEST", "PDPC1302", 1, 3)
    ...
    save_result_list_to_json(result_list, LOCAL_RESULT_FILE)
