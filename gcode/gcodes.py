...
class GCodeArcMove(GCodeMotion):
    ...
    def assert_params(self):
        ...
        # XYZ: at least 1
        if not params_xyz:
            raise GCodeParameterError("no XYZ parameters set for destination: %r" % str(self))   # arc_gcode _MR_
        # IJK or R: only in 1 group
        if params_ijk and params_r:
            raise GCodeParameterError("both IJK and R parameters defined: %r" % str(self))
        # IJKR: at least 1
        if not params_ijkr:
            raise GCodeParameterError("neither IJK or R parameters defined: %r" % str(self))

        # --- Parameter Values
        if params_r and (self.R == 0):
            raise GCodeParameterError("cannot plot a circle with a radius of zero: %r" % str(self))
