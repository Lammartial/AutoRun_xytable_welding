...
def monitor_light_curtain(adam, logger, get_state):
    previous_state = not adam.get_digital_input(index=3)

    while True:
        current_state = not adam.get_digital_input(index=3)
        ...
        if current_state != previous_state:
            ...
            if current_state:
                logger.log_event(
                    ...
                    details="Light curtain switched ON",
                )
            else:
                logger.log_event(
...
class EventLogger:
    ...
    def __init__(self, filename="production_events_line3_manual.csv"):
...
def wait_for_move_or_grind(adam) -> str:
    """Block until either Move or Grind button is pressed.
    Returns 'move' or 'grind' so the caller can route accordingly."""
    print("Press MOVE to continue, or GRIND to start the electrode grinding process...")
    while True:
        if adam.get_digital_input(index=1):   # Move button
            if is_light_curtain_activated(adam):
                ...
                print("Light curtain deactivated! Please activate LC first, then press the Move/Grind button again.")
            
        elif adam.get_digital_input(index=0):   # Grind button
            if is_light_curtain_activated(adam):
                print("Grind button pressed! -> Stop at current position and enter grind flow")
                return 'grind'
            else:
                print("Light curtain deactivated! Please activate LC first, then press the Move/Grind button again.")            
        sleep(0.1)
    ...
    match current_state:
        ...
        case StageStates.START:
            if xy_table.is_connected():    
                ...
                # Set recovery context for grind event
                if table_index == -1:  # When xytable starts at home
                    target_x, target_y = 0, 0
                elif table_index == 64:
                    target_x, target_y = WORKER_POSITION_X, WORKER_POSITION_Y
                else:
                    target_x, target_y = table_of_positions[table_index - 2]

                xy_table._recovery_context = {
                    "target": (target_x, target_y),
                    "return_state": StageStates.START,
                    "grind_phase": 0,
                    "advance_table": False,
                }
                
                # if is_move_button_pressed(adam):
                choice = wait_for_move_or_grind(adam)
                ...
                if choice == "move":
                    ...
                    if grind_request_event.is_set():
                        ...
                        logger.log_event(
                            ...
                            details="Operator pressed the Grind button"
                        )
            ...
            if isinstance(x, str):
                if x == "PAUSE" and y is not None:
                    ...
                    if not is_connected:  
                        ...
                        print(f"⚠️  Power cut while moving to X={target_x} Y={target_y}. Entering recovery.")
                        _next_state = StageStates.EMERGENCY_HANDLING
                        ...
                        print(f"Welding position {welding_position}.")
                        print("Wait until welding is done.")
                        _next_state = StageStates.WAIT_WELDING_DONE     
                    ...
                    if grind_request_event.is_set():
                        ...
                        if not is_move_successfully:
                            logger.log_event(
                                ...
                                target=(x, y),
                                reason="Power/safety interruption",
                                details="Power was interrupted during the xytable movement"
                            )
        ...
        case StageStates.WAIT_WELDING_DONE:
            ...
            # if is_light_curtain_activated(adam) or is_emergency_button_pressed(adam):
                ...
                if welder.is_toggle_bit_changed():
                    ...
                    _next_state = StageStates.CHECK_WELD_RESULT

        case StageStates.CHECK_WELD_RESULT:
...
def test_combined_controllers(resource_str_aws: str, resource_str_xy: str, resource_str_adam: str, logger: EventLogger):
    ...
    light_curtain_thread = Thread(
        ...
        args=(adam, logger, lambda: state_of_machine),
        daemon=True,
    )
...
if __name__ == "__main__":
    ...
    _log = getLogger(__name__, DEBUG)
    production_logger = EventLogger("production_events_line3_manual.csv")
    tic = perf_counter()
